...
function handlerEntryFromYaml(raw: unknown): HandlerEntry {
    if (!raw || typeof raw !== 'object') { throw new Error('Invalid handler entry'); }
...
function appConfigFromYaml(raw: YamlValue): Omit<AppConfig, 'handlers'> & { handlers: HandlerEntry[] } {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    ...
    const extensions = Array.isArray(r['extensions'])
        ? (r['extensions'] as unknown[]).map(String)
        : [];
    return {
        name:        String(r['name']        ?? 'unnamed'),
        version:     r['version']     ? String(r['version'])     : undefined,
