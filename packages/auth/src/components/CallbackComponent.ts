...
export class CallbackComponent extends FlowComponent {
    ...
    private async _process(req: CallbackRequest): Promise<void> {
        try {
            ...
            // Upsert identity
            let identity = await this._identities.findByProvider(req.provider, profile.providerUserId);
            if (!identity) {
                identity = await this._identities.create({
