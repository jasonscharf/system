...
export class UserIdentityRepository {
    ...
    async create(input: Omit<UserIdentityEntity, 'id' | 'iri' | 'createdAt' | 'updatedAt'>): Promise<UserIdentityEntity> {
        const id  = newId();
        const now = new Date();
        ...
        await this._store.insertMany(quads);
        return { id, iri: sub.value, ...input, createdAt: now, updatedAt: now };
    }

    async findByProvider(provider: OAuthProvider, providerUserId: string): Promise<UserIdentityEntity | null> {
        const byProvider = await this._store.find({
    ...
    async findByUserId(userId: string): Promise<UserIdentityEntity[]> {
        const userIri  = iriFor('user', userId);
        const byUser   = await this._store.find({ predicate: identityOfIRI, object: userIri, graph: AUTH_GRAPH });
    ...
    async updateTokens(id: string, tokens: Pick<UserIdentityEntity, 'accessToken' | 'refreshToken' | 'tokenExpiresAt'>): Promise<void> {
        const sub = iriFor('identity', id);
        const now = new Date();

        await this._store.delete({ subject: sub, predicate: accessTokenIRI,    graph: AUTH_GRAPH });
        await this._store.delete({ subject: sub, predicate: refreshTokenIRI,   graph: AUTH_GRAPH });
        await this._store.delete({ subject: sub, predicate: tokenExpiresAtIRI, graph: AUTH_GRAPH });
        await this._store.delete({ subject: sub, predicate: updatedAtIRI,      graph: AUTH_GRAPH });

        await this._store.insert({ subject: sub, predicate: accessTokenIRI, object: literal(tokens.accessToken, XSD_STRING), graph: AUTH_GRAPH });
        if (tokens.refreshToken)  { await this._store.insert({ subject: sub, predicate: refreshTokenIRI,  object: literal(tokens.refreshToken,            XSD_STRING),   graph: AUTH_GRAPH }); }
        if (tokens.tokenExpiresAt) { await this._store.insert({ subject: sub, predicate: tokenExpiresAtIRI, object: literal(tokens.tokenExpiresAt.toISOString(), XSD_DATETIME), graph: AUTH_GRAPH }); }
