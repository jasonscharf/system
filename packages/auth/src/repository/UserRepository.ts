...
export class UserRepository {
    ...
    async create(ctx: ServerContext, input: Pick<UserEntity, 'email' | 'displayName' | 'avatarUrl'>): Promise<UserEntity> {
        ...
        await this._store.insertMany(ctx, [
            ...
            ...(input.displayName ? [{ subject: sub, predicate: displayNameIRI, object: literal(input.displayName, XSD_STRING), graph: AUTH_GRAPH }] : []),
            ...(input.avatarUrl   ? [{ subject: sub, predicate: avatarUrlIRI,   object: literal(input.avatarUrl,   XSD_STRING), graph: AUTH_GRAPH }] : []),
        ]);
