...
export class EntityQuery<H extends EntityHandle[]> {
    ...
    async all(): Promise<EntityRecord[]> {
        ...
        let candidateIris = await this._allEntityIris();
        ...
        for (const f of eqFilters) {
            candidateIris = await this._applyEqFilter(candidateIris, f);
        }

        // Step 3: hydrate
        let records = await this._es.hydrateMany(this._schema, candidateIris, this._handles);
    ...
    async first(): Promise<EntityRecord | null> {
    ...
    async count(): Promise<number> {
        ...
        let iris = await this._allEntityIris();
        for (const f of this._filters.filter(ff => ff.op === '=')) {
            iris = await this._applyEqFilter(iris, f);
        }
        return iris.length;
    }
    ...
    private async _allEntityIris(): Promise<string[]> {
        const quads = await this._store.find({ predicate: RDF_TYPE, object: this._schema.typeIRI });
        return quads.map(q => (q.subject as IRI).value);
    }

    private async _applyEqFilter(iris: string[], f: Filter): Promise<string[]> {
        ...
        const propQuads  = await this._store.find({ predicate: propIri, object: valueNode });
        ...
        for (const pgVal of matchingPg) {
            ...
            const entQuads = await this._store.find({ predicate: TERN_PROP_GROUP, object: pgNode });
 ...
 *   const results = await entities(store).find(UserSchema, '*').all();
 */
