...
export function readShaclShapes(triples: Triple[]): ShaclShapes {
    ...
    for (const { subject, predicate, object } of triples) {
        if (!predicate.equals(RDF_TYPE)) { continue; }
        if (!(object instanceof IRI) || !object.equals(SH_NODE_SHAPE)) { continue; }
    ...
    for (const { subject, predicate, object } of triples) {
        ...
        if (shape) {
            if (predicate.equals(SH_TARGET_CLASS)) {
                ...
                if (objKey) { shape.targetClass = objKey; }
            } else if (predicate.equals(SH_PROPERTY)) {
                const propKey = termKey(object);
                if (propKey) {
                    if (!shapePropLinks.has(subjKey)) { shapePropLinks.set(subjKey, []); }
                    shapePropLinks.get(subjKey)!.push(propKey);
                    if (!propShapeMap.has(propKey)) { propShapeMap.set(propKey, {}); }
                }
            } else if (predicate.equals(SH_CLOSED)) {
                if (literalStr(object) === 'true') { shape.closed = true; }
            }
        }
    }
    ...
    for (const { subject, predicate, object } of triples) {
        const subjKey = termKey(subject);
        if (!subjKey || !propShapeMap.has(subjKey)) { continue; }
    ...
    for (const [shapeKey, propKeys] of shapePropLinks) {
        ...
        for (const propKey of propKeys) {
            ...
            if (ps?.path) { nodeShape.properties.push(ps as ShaclPropertyShape); }
        }
    }
    ...
    for (const shape of nodeShapes.values()) {
        if (shape.targetClass) { byTargetClass.set(shape.targetClass, shape); }
    }
