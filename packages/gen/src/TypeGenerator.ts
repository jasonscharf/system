...
function renderPropertyAugmented(
    ...
    const lines: string[] = [];
    if (prop.comment) { lines.push(`${indent}/** ${prop.comment} */`); }
...
export function generateAugmentedTypes(
    ...
    for (const cls of ontology.classes.values()) {
        ...
        if (pkg) {
            if (!importsByPkg.has(pkg)) { importsByPkg.set(pkg, new Set()); }
    ...
    const localClasses = [...ontology.classes.values()]
        .filter(cls => cls.iri.startsWith(localNamespace));

    for (const cls of localClasses) {
        if (cls.comment) { lines.push(`/** ${cls.comment} */`); }
    ...
    const externalClassList = [...ontology.classes.values()]
        .filter(cls => !cls.iri.startsWith(localNamespace));
    ...
    const localProps = [...ontology.properties.values()]
        .filter(p => p.iri.startsWith(localNamespace));
