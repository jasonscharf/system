...
export function ComponentShowcaseView({ model }: Props): React.ReactElement {
    ...
    const handleEcho = useCallback(() => {
        if (echoInput.trim()) {
            void model.echo(echoInput.trim());
        }
    }, [model, echoInput]);
    const handleEchoKey = useCallback(
        ...
        <div className="showcase">
            <header className="showcase__header">
                ...
                <p className="showcase__subtitle">
                    A React component that is also a Flow-based programming node. Handler endpoints
                    are registered via YAML and Turtle config files.
                </p>
            </header>
            ...
            <section className="showcase__section">
                ...
                <div
                    className={`status-badge ${connected ? "status-badge--ok" : "status-badge--err"}`}
                >
            ...
            <section className="showcase__section">
                <h2>
                    Ping <span className="endpoint-badge">tern:ping</span>
                </h2>
            ...
            <section className="showcase__section">
                <h2>
                    Echo <span className="endpoint-badge">tern:echo</span>
                </h2>
                <p className="showcase__hint">
                    Sample endpoint registered via <code>config/extensions/echo.yaml</code>. The
                    server uppercases your message and echoes it back.
                </p>
                <div className="showcase__row">
                    <input
            ...
            <section className="showcase__section">
                <h2>
                    Triple Store <span className="endpoint-badge">tern:triple.stats</span>
                </h2>
                <button type="button" className="btn" onClick={handleStats} disabled={!connected}>
                    Refresh Stats
                </button>
                {stats !== null && (
                    <table className="stats-table">
                        <tbody>
                            <tr>
                                <th>Namespaces</th>
                                <td>{stats.namespaces}</td>
                            </tr>
                            <tr>
                                <th>Names</th> <td>{stats.names}</td>
                            </tr>
                            <tr>
                                <th>Nodes</th> <td>{stats.nodes}</td>
                            </tr>
                            <tr>
                                <th>Edges</th> <td>{stats.edges}</td>
                            </tr>
                        </tbody>
                    </table>
                )}
