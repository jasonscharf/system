...
export class OrganizationRepository {
	...
	async create(
		...
		await this._store.insertMany(ctx, [
			{ subject: sub, predicate: RDF_TYPE, object: OrganizationIRI, graph: TENANCY_GRAPH },
			{
				subject: sub,
			...
			{
				subject: sub,
				predicate: orgCreatedAtIRI,
				object: literal(now.toISOString(), XSD_DATETIME),
				graph: TENANCY_GRAPH,
			},
			{
				subject: sub,
				predicate: orgUpdatedAtIRI,
				object: literal(now.toISOString(), XSD_DATETIME),
				graph: TENANCY_GRAPH,
			},
		]);
	...
	private _fromQuads(
		...
		return {
			...
			createdAt: new Date(get(orgCreatedAtIRI) ?? 0),
			updatedAt: new Date(get(orgUpdatedAtIRI) ?? 0),
		};
	}
}
