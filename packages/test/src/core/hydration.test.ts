...
describe("TripleStream", () => {
    ...
    it("filter() keeps matching triples", async () => {
        const stream = new TripleStream(mockSource([t1, t2]));
        const filtered = await stream
            .filter((t) => t.subject instanceof IRI && t.subject.equals(new IRI("http://s1")))
            .collect();
...
describe("FileSource", () => {
    ...
    beforeEach(async () => {
        tmpDir = await mkdtemp(join(tmpdir(), "tern-hydration-"));
    });
    afterEach(async () => {
...
describe("HttpSource", () => {
    ...
    it("streams triples from HTTP response", async () => {
        ...
        const parser = vi.fn(async function* () {
            yield t1;
        });
        expect(
            await new TripleStream(new HttpSource("http://example.org/", parser)).collect(),
        ).toEqual([t1]);
    });
...
describe("SocketSource", () => {
    ...
    class MockWebSocket {
        ...
        emit(event: string, data?: unknown) {
            for (const fn of this.listeners[event] ?? []) {
                fn(data !== undefined ? { data } : undefined);
            }
        }
    }
    ...
    it("yields triples from socket messages", async () => {
        ...
        const consuming = (async () => {
            for await (const t of source.stream()) {
                results.push(t);
            }
        })();
