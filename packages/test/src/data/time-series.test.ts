...
// Timestamp columns are generated by the database server. When the DB runs in a
// separate process/container its clock can differ from the test host's by a few
// ms, so timestamp-vs-host-clock bounds are checked with this tolerance.
...
// Postgres returns JSONB columns as already-parsed objects; SQLite returns text.
...
for (const db of providers) {
    ...
    describe(`TripleStore — node timestamps (${db.name})`, () => {
        ...
        it("IRI nodes have created_at and updated_at in UTC", async () => {
            ...
            expect(createdMs).toBeGreaterThanOrEqual(before - CLOCK_SKEW_MS);
            expect(createdMs).toBeLessThanOrEqual(after + CLOCK_SKEW_MS);
    ...
    describe(`TripleStore — soft delete (${db.name})`, () => {
        ...
        it("delete() marks the edge is_deleted=true and sets deleted_at", async () => {
            ...
            expect(deletedMs).toBeGreaterThanOrEqual(before - CLOCK_SKEW_MS);
            expect(deletedMs).toBeLessThanOrEqual(after + CLOCK_SKEW_MS);
        });
    ...
    describe(`TripleStore — temporal invariants (${db.name})`, () => {
        ...
        it("updated_at = deleted_at when soft-deleted", async () => {
            ...
            // Postgres returns Date objects, SQLite strings — compare by value.
            expect(new Date(row.updated_at).getTime()).toBe(new Date(row.deleted_at).getTime());
        });
    ...
    describe(`EntityStore — soft-delete integration (${db.name})`, () => {
        ...
        beforeEach(async () => {
            ...
            es = new EntityStore(store, undefined, TEST_CIPHER);
            ctx = buildServerContext(store, { trx, cipher: TEST_CIPHER });
        });
        afterEach(async () => {
            await trx.rollback();
            await assertEmptyStore(knex);
        ...
        it("EntityStore.update produces two history rows for the changed predicate", async () => {
            ...
            // displayName is PII: history rows hold ciphertext at rest (findHistory
            // bypasses the EntityStore decryption seam), so decrypt before comparing.
            const decrypt = (q?: typeof old) =>
                TEST_CIPHER.decrypt(TEST_CIPHER.currentKeyId, (q?.object as Literal).value);
            expect(decrypt(old)).toBe("Old");
            expect(decrypt(current)).toBe("New");
        });
