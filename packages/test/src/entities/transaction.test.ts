...
interface DbProvider { name: string; isPg: boolean; create(): Promise<Knex>; }

const providers: DbProvider[] = [
    { name: 'SQLite', isPg: false, create: () => createDataContext({ client: 'sqlite', filename: ':memory:' }) },
];
...
for (const db of providers) {
    ...
    describe(`EntityStore — transaction semantics (${db.name})`, () => {
        ...
        beforeEach(async () => { env = await setup(db); });
        ...
        it('testCollectionGetChainsIntoCallerTrx', async () => {
            ...
            await es.collectionPush(txCtx, taggedSchema, rec.id, tagHandle, 'tags', 'alpha', 'beta');
    ...
    describe(`EntityQuery — transaction semantics (${db.name})`, () => {
        ...
        it('testQueryAllChainsRollbackRemovesResults', async () => {
            ...
            expect((await entities(store).find(schema, '*').all(txCtx)).map(r => r.id)).toContain(rec.id);
        ...
        it('testQueryIsolationOnPostgres', async () => {
            ...
            const results = await entities(store).find(schema, '*').all({});
            expect(results.every(r => r.groups[TrxHandle.id]?.['title'] !== 'isolated-query')).toBe(true);
