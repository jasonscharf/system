...
const USER_CREATED = "urn:tern:core:auth:user.created";
const USER_UPDATED = "urn:tern:core:auth:user.updated";
...
function makeEvent(type: string, payload: unknown, id = uniqueId()): DomainEvent<unknown> {
    return { id, type, source: "urn:tern:test", timestamp: Date.now(), payload };
}
