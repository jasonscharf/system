...
beforeAll(async () => {
    ...
    src = generateSchemas(
        ...
        { localNamespace: "urn:tern:core:rbac:", iriImport: "@jasonscharf/core" },
    );
});
...
describe("generateSchemas — real rbac.ttl", () => {
    ...
    it("keeps datatype properties as literal properties (no fooId)", () => {
        expect(src).toContain('tenantName: new IRI("urn:tern:core:rbac:tenantName")');
        expect(src).toContain('permissionKey: new IRI("urn:tern:core:rbac:permissionKey")');
    ...
    it("models polymorphic object properties (no rdfs:range) as targetless edges", () => {
        ...
        expect(src).toContain(
            'hasPrincipal: { predicate: new IRI("urn:tern:core:rbac:hasPrincipal"), cardinality: "many", direction: "out" }',
        );
        expect(src).toContain(
            'hasScope: { predicate: new IRI("urn:tern:core:rbac:hasScope"), cardinality: "many", direction: "out" }',
        );
