...
describe('EnvSecretsProvider', () => {
    ...
    it('getRequired() throws when env var is absent', async () => {
        await expect(provider.getRequired('__NO_SUCH_VAR_XYZ__')).rejects.toThrow('__NO_SUCH_VAR_XYZ__');
    });

    it('getRequired() throws when env var is empty string', async () => {
        process.env['__TEST_REQUIRED__'] = '';
        await expect(provider.getRequired('__TEST_REQUIRED__')).rejects.toThrow('__TEST_REQUIRED__');
    });
});
...
describe('CachedSecretsProvider', () => {
    ...
    it('close() clears cache and calls inner close()', async () => {
        const inner  = new InMemorySecretsProvider({ X: 'v' }) as ISecretsProvider & { close?: () => Promise<void> };
    ...
    it('maps underscore key names to hyphenated vault names', async () => {
        ...
        class TestableProvider extends AzureKeyVaultProvider {
            toVaultName(key: string): string {
                return (this as unknown as { _toVaultName: (k: string) => string })._toVaultName(key);
            }
        }
        ...
        expect(p.toVaultName('TERN_PG_PASSWORD')).toBe('tern-pg-password');
...
describe('SecretsManager', () => {
    ...
    it('close() calls provider close if available', async () => {
        const inner = new InMemorySecretsProvider() as ISecretsProvider & { close?: () => Promise<void> };
        inner.close = vi.fn(async () => {});
