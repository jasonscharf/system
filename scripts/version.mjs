...
const PACKAGES = [
    'api', 'app', 'auth', 'core', 'data',
    'entities', 'flow', 'gen', 'server', 'vaults',
];
...
function taggedVersion() {
    ...
    const candidates = [];
    if (isTagPush) { candidates.push(refName); }
    try {
...
console.log(`run=${runNumber}  ref=${refType}:${refName}  tagVer=${tagVer ? `${tagVer.major}.${tagVer.minor}` : '(none)'}  suffix="${suffix}"`);

for (const pkg of PACKAGES) {
    const pkgPath = join(ROOT, 'packages', pkg, 'package.json');
    const json    = JSON.parse(readFileSync(pkgPath, 'utf8'));

    const { major, minor } = tagVer ?? (() => {
